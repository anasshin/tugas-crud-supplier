...
<body class="">
    <div class="container mx-auto py-8">
        <div class="max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <form action="{{ route('supplier.update', $data_supplier->id) }}" method="POST" class="px-6 py-4">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nama_supplier" id="nama_supplier"
                        value="{{ old('nama_supplier', $data_supplier->nama_supplier) }}"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="alamat" id="alamat" rows="3"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200"
                        required>{{ old('alamat', $data_supplier->alamat) }}</textarea>
                    @error('alamat')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="telepon" id="telepon"
                        value="{{ old('telepon', $data_supplier->telepon) }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200"
                        required>
                    @error('telepon')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div class="mb-6">
                    ...
                    <input type="email" name="email" id="email"
                        value="{{ old('email', $data_supplier->email) }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200"
                        required>
                    @error('email')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
